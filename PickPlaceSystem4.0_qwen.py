...
from qwen_remote_client import classify_one_image, set_remote_config
...
# 放置目标Pose6
TARGET_PLACE_POSE6 = [600, 56, -380, 177.676, -0.621, -132.258]
...
class PickPlaceSystem:
    ...
    def estimate_pick_once(self, refine_iter=5):
        ...
        if ret is None:
            print("[VISION] 识别失败：ret is None")
            return {"status": "fail", "data": None}

        if "cTo" not in ret:
            print("[VISION] 识别失败：结果中没有 cTo")
            return {"status": "fail", "data": None}

        if ret["cTo"] is None:
            print("[VISION] 识别失败：cTo is None")
            return {"status": "fail", "data": None}
        ...
        # back 才允许抓，front 不抓
        # =========================
        qwen_image_path = None

        if "masked_rgb_path" in ret and ret["masked_rgb_path"] is not None:
            qwen_image_path = ret["masked_rgb_path"]

        if qwen_image_path is None or (not os.path.exists(qwen_image_path)):
            print("[QWEN] 没有可用的物体裁剪图，跳过本轮")
            return {"status": "fail", "data": None}

        print(f"[QWEN] 使用裁剪图分类: {qwen_image_path}")
        qwen_label = classify_one_image(qwen_image_path)

        if qwen_label is None:
            qwen_label = "unknown"

        print(f"[QWEN] 分类结果: {qwen_label}")

        # 只有 back 才允许抓
        if qwen_label != "back":
            print("[QWEN] 判定为 front 或 unknown，不抓取")
            return {"status": "fail", "data": None}

        # 原有位姿安全检查继续保留
        if not is_cto_mm_z_axis_up(ret["cTo"]):
            print("[VISION] 位姿检查失败：z轴方向不对")
            return {"status": "fail", "data": None}

        pick_pose6 = matrix_to_pose6(self.compute_pick_pose(ret["cTo"]))
        ret["pick_pose6"] = pick_pose6
        ret["qwen_label"] = qwen_label

        print("[VISION] Qwen判定为 back，允许抓取")
...
def main():
    ...
    parser.add_argument('--remote_api_base', type=str, default='http://127.0.0.1:8000/v1')
    parser.add_argument('--remote_timeout', type=int, default=120)
    parser.add_argument('--remote_model_name', type=str, default='auto')
    parser.add_argument('--remote_api_key', type=str, default='EMPTY')
