...
class OrbbecCamera:
    def __init__(self):
        ...
        #     try:
        #         depth_profile = profile_list.get_video_stream_profile(640, 480, OBFormat.Y16, 15)
        #     except OBError as e:
        #         print(e)
        #         depth_profile = profile_list.get_video_stream_profile(640, 480, OBFormat.Y16, 15)
        ...
        # 等价配置方案二
        profile_list = self.pipeline.get_stream_profile_list(OBSensorType.COLOR_SENSOR)
        color_profile = profile_list.get_video_stream_profile(640, 480, OBFormat.RGB, 30)#get_default_video_stream_profile()
        # color_profile = profile_list.get_video_stream_profile(1280,720, OBFormat.RGB, 15)#get_default_video_stream_profile()
        config.enable_stream(color_profile)

        profile_list = self.pipeline.get_stream_profile_list(OBSensorType.DEPTH_SENSOR)
        depth_profile = profile_list.get_video_stream_profile(640, 480, OBFormat.Y16, 30)#get_default_video_stream_profile()
        # depth_profile = profile_list.get_video_stream_profile(1280,720, OBFormat.Y16, 15)#get_default_video_stream_profile()
        config.enable_stream(depth_profile)
        ...
        while self.running:
            try:
                ...
                if depth_frame is None:
                    continue
                #Both Color and Depth Frames are Available, Ready to Aligh
                ...
                self.current_depth_map = depth_data.copy()
                ...
                if depth_image is None:
                    print("failed to convert frame to depth")
                    continue
...
#     depth = cv2.imread("depth.tiff",cv2.IMREAD_UNCHANGED)
#     pass
#     cam = OrbbecCamera()
#     while True:
#         cam.read()
#         print(cam.current_depth.shape)
#         cv2.imshow("Depth Viewer", cam.current_depth)
#         cv2.imshow("Color Viewer", cam.current_color)
#         key = cv2.waitKey(1)
#         if key == ord('q') or key == ESC_KEY:
#             cv2.imwrite("depth.tiff", cam.current_depth_map)
#             cv2.imwrite("color.png", cam.current_color)
#             break
#         time.sleep(0.001)
#
#     cam.stop()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if key == ord('s'):
            ...
            cv2.imwrite(depth_path, cam.current_depth_map)  # 原始深度
            cv2.imwrite(color_path, cam.current_color)      # 彩色图
...
if __name__ == "__main__":
    ...
    output_dir = "/home/robot/6d/FoundationPose2.0/sanpshot_depth_rgb"
    os.makedirs(output_dir, exist_ok=True)
